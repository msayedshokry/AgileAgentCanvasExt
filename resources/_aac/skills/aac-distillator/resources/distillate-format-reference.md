...
**Brief says:**
```
aac-help must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
aac-help must always be included as a base skill in every bundle/install
(solves discoverability problem)
```

**Distillate keeps the more contextual version:**
```
- aac-help: always included as base skill in every bundle (solves discoverability)
```
...
type: aac-distillate
sources:
  - "product-brief-aac-next-gen-installer.md"
  - "product-brief-aac-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
- Three layers: self-describing plugins (aac-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via aac-setup skill
...
- Plugins: skill bundles with Anthropic plugin standard as base format + aac-manifest.json extending for BMAD-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"bmm","replaces-skill":"aac-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- aac-setup: global skill scanning installed aac-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- aac-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
- aac-setup discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- aac-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, aac-setup, aac-update, Vercel CLI integration, NPX installer, zip bundles, migration path
...
- Entry: `tools/installer/aac-cli.js` (Commander.js) → `tools/installer/core/installer.js`
...
- Skills already use directory-per-skill layout; aac-manifest.json sidecars exist but are not source of truth
...
- aac-update mechanics: diff/replace? Preserve user customizations?
...
- aac-manifest.json as open standard submission to Agent Skills governance?
...
- CI/CD integration: aac-setup as pipeline one-liner increases stickiness
