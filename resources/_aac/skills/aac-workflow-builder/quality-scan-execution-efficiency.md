...
- `aac-manifest.json` — Stage ordering, dependencies
...
1. **Parallel read batch:** Read SKILL.md, aac-manifest.json (if present), and all prompt files at skill root — in a single parallel batch
...
### Scan Completeness
- Did I read SKILL.md, aac-manifest.json (if present), and EVERY prompt file?
