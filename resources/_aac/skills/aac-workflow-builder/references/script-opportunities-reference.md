...
**What:** Compare prompt file frontmatter against the skill's `aac-skill-manifest.yaml`.
...
**Implementation:** Python, reads `aac-skill-manifest.yaml` and all prompt `.md` files at skill root.
...
**Checks:**
- Parse `aac-skill-manifest.yaml` for external skill references
...
- Cross-reference against variables loaded by `aac-init` or defined in config
