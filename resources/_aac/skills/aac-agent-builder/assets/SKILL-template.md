---
name: aac-{module-code-or-empty}-agent-{agent-name}
...
/aac-{agent-skill-name} --headless

# Autonomous - specific task
/aac-{agent-skill-name} --headless:refine-memories
```
{/if-headless}
...
1. **Load config via aac-init skill** — Store all returned vars for use:
...
{/if-no-autonomous}
   ...
   - **Load manifest** — Read `aac-manifest.json` to set `{capabilities}` list of actions the agent can perform (internal prompts and available skills)
   ...
   - **Present menu from aac-manifest.json** — Generate menu dynamically by reading all capabilities from aac-manifest.json:
   ...
   **Available capabilities:**
   (For each capability in aac-manifest.json capabilities array, display as:)
   {number}. [{menu-code}] - {description} → {prompt}:{name} or {skill}:{name}
   ```

   **Menu generation rules:**
   - Read aac-manifest.json and iterate through `capabilities` array
...
**CRITICAL Handling:** When user selects a code/number, consult the aac-manifest.json capability mapping:
