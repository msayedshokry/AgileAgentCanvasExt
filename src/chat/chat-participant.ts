...
export class AgileAgentCanvasChatParticipant {
    ...
    /**
     * Build a concise codebase knowledge graph context block for the LLM system prompt.
     *
     * Tiered strategy:
     *   1. ARCH_INDEX.md (preferred) — intentionally budgeted ~1500 tokens, high relevance
     *   2. Fallback — truncated community list + partial GRAPH_REPORT.md (legacy behaviour)
     */
    private buildGraphContext(): string | null {
        ...
        // ── Tier 1: ARCH_INDEX.md (preferred) ───────────────────────────────
        const archIndex = loadArchIndexMarkdown(root);
        if (archIndex) {
            return `## Codebase Knowledge Graph (graphify)\n\n${archIndex}`;
        }

        // ── Tier 2: Fallback (ARCH_INDEX not yet generated) ──────────────────
        // Kept for backward-compat; ARCH_INDEX is generated by `graphify extract`.
        ...
        const lines: string[] = [
            ...
            `_Tip: run \`graphify extract .\` to generate a richer ARCH_INDEX (better token efficiency)_`,
            ``,
        ];
...
Output ONLY the JSON, no explanation.`;
     ...
     *
     * With no prompt: shows a quick-start menu of top categories.
     * With a prompt:  uses the LLM to pick the top 3-5 matching skills and presents them
     *                 as both a CLI-readable markdown list and VS Code command suggestions.
     */
    private async handleHelpCommand(
        ...
        // If no prompt — show quick-start guide
        if (!prompt.trim()) {
            stream.markdown([
                '## Agile Agent Canvas — What do you want to do?\n',
                'Describe your task after `/help` and I\'ll recommend the best skills and agents.\n',
                'For example: `@agileagentcanvas /help I want to write test cases`\n\n',
                '### Quick-start categories\n',
                '| Category | Command |',
                '|---|---|',
                '| Planning & requirements | `/vision`, `/requirements`, `/epics` |',
                '| Development | `/dev`, `/stories`, `/quick` |',
                '| Testing & quality | `/review-code`, `/ci` |',
                '| Architecture | `/apply`, `/context` |',
                '| UX & design | `/ux`, `/design-thinking` |',
                '| Creativity | `/innovate`, `/solve`, `/story-craft` |',
                '| Workflows | `/workflows` or use the **Workflows FAB** on the canvas |',
                '\n\n> **Tip:** Open the canvas toolbar 🗂️ button to browse and manage all available skills.',
            ].join('\n'));
        ...
        if (manifest.length === 0) {
            stream.markdown('Skills are still loading. Try again in a moment, or use `/workflows` to launch a workflow directly.');
            return { metadata: { command: 'help' } };
        }
        ...
        // Build a compact catalogue string for the LLM (token-efficient)
        const catalogueSummary = manifest
            .map(e => `- ${e.name}${e.isAgent ? ' [agent]' : ''}: ${e.description || '(no description)'}`)
            .join('\n');

        const systemPrompt = [
            'You are a routing assistant for Agile Agent Canvas, a VS Code extension.',
            'Given a user request and a catalogue of available skills/agents,',
            'select the top 3 to 5 most relevant entries and explain briefly why each fits.',
            'Reply ONLY with a valid JSON array (no markdown fences, no extra text), e.g.:',
            '[{"name":"Skill Name","reason":"One sentence why this fits."}]',
        ].join('\n');
        ...
        // Use a null stream to collect the LLM response without showing it raw
        let suggestions: Array<{ name: string; reason: string }> = [];
        try {
            // Build a silent mock stream — we want the return value, not streamed output
            const nullStream: vscode.ChatResponseStream = {
                markdown: () => {},
                anchor: () => {},
                button: () => {},
                filetree: () => {},
            ...
            suggestions = manifest
                ...
                .slice(0, 5)
                .map(e => ({ name: e.name, reason: e.description || 'Matches your request keywords.' }));
        }

        if (suggestions.length === 0) {
            stream.markdown([
                `No strong matches found for: **${prompt}**\n\n`,
                'Try rephrasing, or open the **Skill Catalogue** (🗂️ button in the canvas toolbar) to browse all available skills.',
            ].join(''));
            return { metadata: { command: 'help' } };
        }

        // Stream a CLI-compatible numbered markdown list (works in opencode, claude-code, VS Code)
        stream.markdown(`## Best options for: "${prompt}"\n\n`);
        ...
        stream.markdown([
            ...
            '_Or open the **Skill Catalogue** (🗂️ in the canvas toolbar) to manage and discover more skills._\n',
        ].join(''));
