/**
 * artifact-md-exporter.ts (extension host)
 *
 * Converts an artifact object (received as plain JSON from the webview message)
 * to formatted Markdown for saving to disk.
 *
 * NOTE: This mirrors webview-ui/src/utils/artifact-md-exporter.ts but operates
 * on `any` since Artifact types are in the webview bundle.
 */
...
function frontmatter(artifact: any): string {
    const lines = [
        '---',
        `id: ${artifact.id}`,
        `type: ${artifact.type}`,
    ...
    if (artifact.metadata?.priority) {
        lines.push(`priority: ${artifact.metadata.priority}`);
    }
    lines.push('---\n');
    return lines.join('\n');
}
...
function renderStory(meta: any): string {
    ...
    if (tasks.length > 0) {
        ...
        tasks.forEach(t => {
            const check = doneStatuses.includes(t.status) ? 'x' : ' ';
            md += `- [${check}] ${t.description}\n`;
            (t.subtasks || []).forEach((s: any) => {
                const sc = doneStatuses.includes(s.status) ? 'x' : ' ';
                md += `  - [${sc}] ${s.description}\n`;
            });
        });
        md += '\n';
...
function renderEpic(meta: any): string {
    ...
    // Definition of Done — schema: {items:[{criterion, category, completed, required}], qualityGates:[...]}
    ...
    if (dod) {
        ...
        if (Array.isArray(dod)) {
            ...
            if (qg.length > 0) {
                ...
                qg.forEach((g: any) => { md += `- ${g.passed ? '✓' : '○'} ${g.gate || g.criterion || ''}\n`; });
                md += '\n';
            }
    ...
    // Fit Criteria — {functional:[], nonFunctional:[], security:[], performance:[], usability:[]}
    ...
    if (fc && typeof fc === 'object') {
        ...
        if (categories.length > 0) {
            ...
            categories.forEach(cat => {
                ...
                cat.items.forEach((c: any) => { md += `- ${c.verified ? '✓' : '○'} ${c.criterion || ''}\n`; });
                md += '\n';
            });
        }
    }
...
function renderRequirement(meta: any): string {
    let md = '';
    if (meta?.rationale) { md += heading(2, 'Rationale'); md += `${meta.rationale}\n\n`; }
...
function renderUseCase(meta: any): string {
    ...
    if (meta?.preconditions?.length) { md += heading(2, 'Preconditions'); md += bulletList(meta.preconditions); }
    ...
    if (meta?.postconditions?.length) { md += heading(2, 'Postconditions'); md += bulletList(meta.postconditions); }
    return md;
}
...
function renderProductBrief(meta: any): string {
    ...
    if (scope) {
        ...
        if (scope.inScope?.length) {
            ...
            scope.inScope.forEach((i: any) => { md += `- ${i.item}${i.priority ? ` *(${i.priority})*` : ''}\n`; });
            md += '\n';
        }
        if (scope.outOfScope?.length) {
            md += heading(3, 'Out of Scope');
            scope.outOfScope.forEach((i: any) => { md += `- ${i.item}${i.reason ? ` — ${i.reason}` : ''}\n`; });
            md += '\n';
        }
    ...
    const smArr: any[] = meta.successMetrics || [];
    if (smArr.length > 0) {
        md += heading(2, 'Success Metrics');
        md += '| Metric | Target | Category |\n|---|---|---|\n';
        smArr.forEach((m: any) => { md += `| ${m.metric} | ${m.target || ''} | ${m.category || ''} |\n`; });
        md += '\n';
    }
    ...
    if (ac) {
        ...
        if (ac.openQuestions?.length) {
            ...
            ac.openQuestions.forEach((q: any) => { md += `- ${q.question}${q.status ? ` *(${q.status})*` : ''}\n`; });
            md += '\n';
        }
    }
...
function renderGeneric(artifact: any): string {
...
export function convertArtifactToMarkdown(artifact: any): string {
    ...
    if (artifact.description) {
        md += `> ${String(artifact.description).replace(/\n/g, '\n> ')}\n\n`;
    }

    switch (artifact.type) {
        case 'story':           md += renderStory(meta); break;
        case 'epic':            md += renderEpic(meta); break;
        case 'requirement':
        case 'nfr':
        case 'additional-req':  md += renderRequirement(meta); break;
        case 'use-case':        md += renderUseCase(meta); break;
        case 'architecture':    md += renderArchitecture(meta); break;
        case 'prd':
        case 'product-brief':   md += renderProductBrief(meta); break;
        default:                md += renderGeneric(artifact); break;
    }
