 ...
 *
 * Strategy:
 * 1. Write a comprehensive guide file to `.agentcanvas-context/.agentcanvas-guide.md`
 ...
 * 4. When Gemini writes output files to `.agentcanvas-context/`, the file watcher
...
export interface AntigravityWorkflowParams {
    ...
    projectRoot: string;
    /** Absolute path to the output folder (e.g. .agentcanvas-context) */
    outputFolder: string;
...
const GUIDE_FILENAME = '.agentcanvas-guide.md';
...
- Do NOT skip workflow checkpoints or pause instructions just to produce output files`
    ...
    if (mode === 'interactive') {
        ...
        return `# AgentCanvas Workflow Guide

> **This file was auto-generated by the AgentCanvas extension.**
...
You are executing a specific workflow task for the AgentCanvas VS Code extension.
...
export async function orchestrateAntigravityWorkflow(
    ...
    if (stream) {
        ...
        stream.markdown(
            ...
            modeAdvice +
            '> When the agent writes output files, AgentCanvas will automatically detect and load them.\n\n'
        );
    }
