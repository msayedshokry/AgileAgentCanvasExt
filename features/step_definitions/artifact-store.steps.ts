...
function getArtifactStore(world: BmadWorld): any {
  if (!world.artifactStore) {
    ...
    const artifactStoreModule = proxyquire("../../src/state/artifact-store", {
      ...
      "./artifact-file-io": {
        resolveArtifactTargetUri: async (opts: any) =>
          world.vscode.Uri.file(`/test/${opts.fileName}`),
        writeJsonFile: async () => {},
        writeMarkdownCompanion: async (jsonUri: any, mdFilename: string) =>
          world.vscode.Uri.file(`/test/${mdFilename}`),
      },
    ...
    world.artifactStore = new artifactStoreModule.ArtifactStore(
      world.context as any,
    );
  }
...
Given(
  "I initialize a project named {string}",
  function (this: BmadWorld, projectName: string) {
...
Given(
  "I add an epic with title {string}",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
...
Given(
  "I create a story in epic {string}",
  function (this: BmadWorld, epicTitleOrId: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find(
      (e: any) => e.id === epicTitleOrId || e.title === epicTitleOrId,
    );
...
Given(
  "I create a requirement with title {string}",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
...
Given(
  "I select the epic {string}",
  function (this: BmadWorld, epicTitle: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
...
When(
  "I update the epic {string} with title {string}",
  function (this: BmadWorld, oldTitle: string, newTitle: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
...
When(
  "I update the epic {string} with description {string}",
  function (this: BmadWorld, epicTitle: string, description: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.title === epicTitle);
...
When(
  "I update the story with user story:",
  function (this: BmadWorld, dataTable: any) {
    const store = getArtifactStore(this);
...
When(
  "I delete the first story from epic {string}",
  function (this: BmadWorld, epicTitle: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.title === epicTitle);
...
When(
  "I create requirement with title {string}",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
    const req = store.createRequirement();
    store.updateArtifact("requirement", req.id, { title });
...
When(
  "I update the requirement {string} with description {string}",
  function (this: BmadWorld, title: string, description: string) {
    const store = getArtifactStore(this);
...
When(
  "I delete the requirement {string}",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
    const requirements = store.getRequirements();
    const reqIndex = requirements.functional.findIndex(
      (r: any) => r.title === title,
    );
...
Then(
  ...
  function (this: BmadWorld, expectedName: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.projectName,
      expectedName,
      `Expected project name "${expectedName}", got "${state.projectName}"`,
    );
  },
);
...
Then("the vision should have default values", function (this: BmadWorld) {
  ...
  assert.ok(
    state.vision!.productName !== undefined &&
      state.vision!.productName.length > 0,
    "Product name should be set",
  );
  assert.strictEqual(
    state.vision!.status,
    "draft",
    `Expected vision status "draft", got "${state.vision!.status}"`,
  );
});

Then(
  "there should be exactly {int} vision artifact(s)",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
    const state = store.getState();
    ...
    assert.strictEqual(
      visionCount,
      count,
      `Expected ${count} vision artifact(s), got ${visionCount}`,
    );
  },
);

Then(
  "the vision product name should be {string}",
  function (this: BmadWorld, expectedName: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.vision?.productName,
      expectedName,
      `Expected vision productName "${expectedName}", got "${state.vision?.productName}"`,
    );
  },
);

Then(
  "the vision problem statement should be {string}",
  function (this: BmadWorld, expected: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.vision?.problemStatement,
      expected,
      `Expected vision problemStatement "${expected}", got "${state.vision?.problemStatement}"`,
    );
  },
);

Then(
  "the vision target audience should be {string}",
  function (this: BmadWorld, expected: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    // targetAudience might be in targetUsers array or as a string field
    const hasTarget =
      state.vision?.targetUsers?.includes(expected) ||
...
Then(
  "the store should contain {int} epic(s)",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    assert.strictEqual(
      epics.length,
      count,
      `Expected ${count} epic(s), got ${epics.length}`,
    );
  },
);

Then(
  "the epic {string} should exist",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
...
Then(
  "the epic {string} should not exist",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.title === title);
...
Then(
  "the epic {string} should have goal {string}",
  function (this: BmadWorld, title: string, expectedGoal: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.title === title);
    assert.strictEqual(
      epic?.goal,
      expectedGoal,
      `Expected epic "${title}" goal "${expectedGoal}", got "${epic?.goal}"`,
    );
  },
);

Then(
  /^the epic "([^"]*)" should contain (\d+) stor(?:y|ies)$/,
  function (this: BmadWorld, epicTitle: string, count: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.title === epicTitle);
    assert.strictEqual(
      epic?.stories?.length,
      parseInt(count, 10),
      `Expected ${count} stories, got ${epic?.stories?.length}`,
    );
  },
);

Then(
  "the story should have user story {string}",
  function (this: BmadWorld, expected: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    ...
    assert.strictEqual(userStoryStr, expected, `Expected user story "${expected}", got "${userStoryStr}"`);
  },
);

Then(
  "the store should contain {int} requirement(s)",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
    const requirements = store.getRequirements();
    assert.strictEqual(
      ...
      count,
      `Expected ${count} requirement(s), got ${requirements.functional.length}`,
    );
  },
);

Then(
  "the requirement {string} should have description {string}",
  function (this: BmadWorld, title: string, expectedDesc: string) {
    const store = getArtifactStore(this);
    const requirements = store.getRequirements();
    ...
    assert.strictEqual(
      req?.description,
      expectedDesc,
      `Expected requirement "${title}" description "${expectedDesc}", got "${req?.description}"`,
    );
  },
);

Then(
  "the selected artifact type should be {string}",
  function (this: BmadWorld, expectedType: string) {
    const store = getArtifactStore(this);
    const selected = store.getSelectedArtifact?.() || store._selectedArtifact;
    assert.strictEqual(
      selected?.type,
      expectedType,
      `Expected selected type "${expectedType}", got "${selected?.type}"`,
    );
  },
);

Then(
  "the selected artifact should be {string}",
  function (this: BmadWorld, expectedTitle: string) {
    const store = getArtifactStore(this);
    const selected = store.getSelectedArtifact?.() || store._selectedArtifact;
    assert.strictEqual(
      selected?.artifact?.title,
      expectedTitle,
      `Expected selected title "${expectedTitle}", got "${selected?.artifact?.title}"`,
    );
  },
);
...
Then("no artifact should be selected", function (this: BmadWorld) {
  const store = getArtifactStore(this);
  const selected = store.getSelectedArtifact?.() || store._selectedArtifact;
  assert.strictEqual(selected, null, "Expected no artifact to be selected");
});
...
Then(
  "{int} state change event(s) should have been fired",
  function (this: BmadWorld, count: number) {
    const eventCount = this.createdArtifacts.get("_eventCount") || 0;
    assert.ok(
      eventCount >= count,
      `Expected at least ${count} events, got ${eventCount}`,
    );
  },
);
...
When(
  "I create a test case linked to the last story",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    if (!this.lastResult || !this.lastResult.id) {
...
When(
  "I update the test case {string} with title {string}",
  function (this: BmadWorld, tcId: string, title: string) {
    const store = getArtifactStore(this);
...
When(
  "I delete the test case {string}",
  function (this: BmadWorld, tcId: string) {
    const store = getArtifactStore(this);
...
When(
  "I update the test strategy with title {string}",
  function (this: BmadWorld, title: string) {
    const store = getArtifactStore(this);
...
Then(
  "the store should contain {int} test case(s)",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
    const state = store.getState();
    ...
    assert.strictEqual(
      actual,
      count,
      `Expected ${count} test case(s), got ${actual}`,
    );
  },
);

Then(
  "the test case should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
...
Then("the test case should be linked to the story", function (this: BmadWorld) {
  ...
  assert.ok(
    tc.storyId !== undefined,
    `Expected test case to have storyId, got ${JSON.stringify(tc)}`,
  );
});

Then(
  "the test case should be linked to epic {string}",
  function (this: BmadWorld, epicTitle: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    const epics = store.getEpics();
    ...
    assert.strictEqual(
      tc.epicId,
      epic.id,
      `Expected test case epicId to be "${epic.id}", got "${tc.epicId}"`,
    );
  },
);

Then(
  "the test case {string} should have title {string}",
  function (this: BmadWorld, tcId: string, expectedTitle: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    ...
    assert.strictEqual(
      tc.title,
      expectedTitle,
      `Expected title "${expectedTitle}", got "${tc.title}"`,
    );
  },
);
...
Then("the store should contain a test strategy", function (this: BmadWorld) {
  ...
  const state = store.getState();
  assert.ok(
    state.testStrategy !== undefined,
    "Expected test strategy to exist",
  );
});

Then(
  "the test strategy should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.testStrategy?.id,
      expectedId,
      `Expected test strategy id "${expectedId}"`,
    );
  },
);

Then(
  "there should be exactly {int} test strategy",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
    const state = store.getState();
    ...
    assert.strictEqual(
      actual,
      count,
      `Expected ${count} test strategy, got ${actual}`,
    );
  },
);

Then(
  "the test strategy should have title {string}",
  function (this: BmadWorld, expectedTitle: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.testStrategy?.title,
      expectedTitle,
      `Expected test strategy title "${expectedTitle}"`,
    );
  },
);

Then(
  "the store should not contain a test strategy",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.ok(
      state.testStrategy === undefined || state.testStrategy === null,
...
When(
  "I use createEpic to create {int} epics",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
...
When(
  "I delete the epic with id {string}",
  function (this: BmadWorld, id: string) {
    const store = getArtifactStore(this);
...
Then(
  "the created epic should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    assert.ok(this.lastResult, "No epic was created");
    assert.strictEqual(
      this.lastResult.id,
      expectedId,
      `Expected created epic id "${expectedId}", got "${this.lastResult.id}"`,
    );
  },
);

When(
  "I create a use case in epic {string}",
  function (this: BmadWorld, epicId: string) {
    const store = getArtifactStore(this);
...
When(
  "I try to create a use case in epic {string}",
  function (this: BmadWorld, epicId: string) {
    const store = getArtifactStore(this);
    try {
...
When(
  "I create a use case without specifying an epic",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
...
Then(
  "the use case should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    assert.ok(this.lastResult, "No use case was created");
    assert.strictEqual(
      this.lastResult.id,
      expectedId,
      `Expected use case id "${expectedId}", got "${this.lastResult.id}"`,
    );
  },
);

Then(
  /^the epic "([^"]*)" should have (\d+) use cases?$/,
  function (this: BmadWorld, epicId: string, count: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    ...
    assert.strictEqual(
      actual,
      parseInt(count, 10),
      `Expected ${count} use case(s), got ${actual}`,
    );
  },
);

Then(
  /^the epic "([^"]*)" should have (\d+) stor(?:y|ies)$/,
  function (this: BmadWorld, epicId: string, count: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    ...
    assert.strictEqual(
      actual,
      parseInt(count, 10),
      `Expected ${count} story(ies), got ${actual}`,
    );
  },
);

Then(
  "an error should have been thrown with message containing {string}",
  function (this: BmadWorld, text: string) {
    assert.ok(this.lastError, "No error was thrown");
    assert.ok(
      this.lastError.message.includes(text),
      `Error "${this.lastError.message}" should contain "${text}"`,
    );
  },
);
...
Then(
  "the product brief should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      state.productBrief?.id,
      expectedId,
      `Expected productBrief id "${expectedId}", got "${state.productBrief?.id}"`,
    );
  },
);

Then(
  "the store should not contain a product brief",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    const state = store.getState();
...
Then(
  ...
  function (this: BmadWorld, expectedId: string) {
    ...
    assert.strictEqual(
      state.prd?.id,
      expectedId,
      `Expected PRD id "${expectedId}", got "${state.prd?.id}"`,
    );
  },
);
...
Then(
  "the architecture should have id {string}",
  function (this: BmadWorld, expectedId: string) {
    const store = getArtifactStore(this);
    const state = store.getState();
    assert.strictEqual(
      ...
      expectedId,
      `Expected architecture id "${expectedId}", got "${state.architecture?.id}"`,
    );
  },
);

Then(
  "the store should not contain an architecture",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    const state = store.getState();
...
When(
  "I delete artifact type {string} with id {string}",
  function (this: BmadWorld, type: string, id: string) {
    const store = getArtifactStore(this);
...
Given(
  "I link requirement {string} to epic {string}",
  function (this: BmadWorld, reqTitle: string, epicId: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.id === epicId);
    assert.ok(epic, `Epic "${epicId}" not found`);
...
Then(
  "the requirement {string} should not reference epic {string}",
  function (this: BmadWorld, reqTitle: string, epicId: string) {
    const store = getArtifactStore(this);
    const requirements = store.getRequirements();
    const reqId = this.createdArtifacts.get(reqTitle);
    ...
    assert.ok(
      !refs.includes(epicId),
      `Requirement should not reference epic "${epicId}"`,
    );
  },
);

Then(
  "epic {string} should not reference requirement {string}",
  function (this: BmadWorld, epicId: string, reqTitle: string) {
    const store = getArtifactStore(this);
    const epics = store.getEpics();
    const epic = epics.find((e: any) => e.id === epicId);
    ...
    assert.ok(
      !funcReqs.includes(reqId),
      `Epic should not reference requirement "${reqTitle}"`,
    );
  },
);

When(
  "I delete artifact type {string} with id the id of {string}",
  function (this: BmadWorld, type: string, name: string) {
    const store = getArtifactStore(this);
...
Given("I create a test design", function (this: BmadWorld) {
  const store = getArtifactStore(this);
  store.artifacts.set("testDesign", {
    id: "test-design-1",
    title: "Test Design",
    status: "draft",
  });
...
When(
  "I find artifact by id the id of {string}",
  function (this: BmadWorld, name: string) {
    const store = getArtifactStore(this);
    const id = this.createdArtifacts.get(name);
...
Then(
  "the found artifact type should be {string}",
  function (this: BmadWorld, expectedType: string) {
    assert.ok(this.lastResult, "No artifact was found");
    assert.strictEqual(
      this.lastResult.type,
      expectedType,
      `Expected found artifact type "${expectedType}", got "${this.lastResult.type}"`,
    );
  },
);
...
When(
  "I load state with project name {string} and a vision and {int} epics",
  function (this: BmadWorld, name: string, epicCount: number) {
    const store = getArtifactStore(this);
...
When(
  "I merge state with {int} new epic",
  function (this: BmadWorld, count: number) {
    const store = getArtifactStore(this);
...
When(
  "I merge state with a different vision and a PRD",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    store.mergeFromState({
      vision: {
...
Then(
  "the vision product name should not have changed",
  function (this: BmadWorld) {
    const store = getArtifactStore(this);
    const state = store.getState();
    ...
    assert.strictEqual(
      state.vision?.productName,
      "Merge Test",
      `Expected vision productName to remain "Merge Test", got "${state.vision?.productName}"`,
    );
  },
);
...
When(
  "I merge state with {int} new requirement and {int} duplicate",
  function (this: BmadWorld, newCount: number, dupCount: number) {
    const store = getArtifactStore(this);
    ...
    store.mergeFromState({
      requirements: { functional: reqs, nonFunctional: [], additional: [] },
    });
  },
);

When(
  "I merge state with {int} new test case and {int} duplicate",
  function (this: BmadWorld, newCount: number, dupCount: number) {
    const store = getArtifactStore(this);
    const tcs: any[] = [];
...
Then("the refine context should not be null", function (this: BmadWorld) {
  const store = getArtifactStore(this);
  assert.ok(
    store.getRefineContext() !== null,
    "Refine context should not be null",
  );
});
...
Then("the refine context should be null", function (this: BmadWorld) {
  const store = getArtifactStore(this);
  assert.strictEqual(
    store.getRefineContext(),
    null,
    "Expected refine context to be null",
  );
});

When(
  "I set pending workflow launch with trigger {string}",
  function (this: BmadWorld, trigger: string) {
    const store = getArtifactStore(this);
    store.setPendingWorkflowLaunch({
      triggerPhrase: trigger,
      workflowFilePath: "/test/wf.md",
    });
  },
);

Then(
  "the pending workflow launch should have trigger {string}",
  function (this: BmadWorld, trigger: string) {
    const store = getArtifactStore(this);
    const launch = store.getPendingWorkflowLaunch();
    assert.ok(launch, "Pending workflow launch should exist");
    assert.strictEqual(
      launch.triggerPhrase,
      trigger,
      `Expected triggerPhrase "${trigger}", got "${launch.triggerPhrase}"`,
    );
  },
);
...
Then("the pending workflow launch should be null", function (this: BmadWorld) {
  const store = getArtifactStore(this);
  assert.strictEqual(
    store.getPendingWorkflowLaunch(),
    null,
    "Expected pending workflow launch to be null",
  );
});
...
Then("hasSelection should return false", function (this: BmadWorld) {
  ...
  assert.strictEqual(
    store.hasSelection(),
    false,
    "Expected hasSelection() to return false",
  );
});
...
Then("hasSelection should return true", function (this: BmadWorld) {
  ...
  assert.strictEqual(
    store.hasSelection(),
    true,
    "Expected hasSelection() to return true",
  );
});
...
Then(
  "the markdown should contain {string}",
  function (this: BmadWorld, text: string) {
    assert.ok(typeof this.lastResult === "string", "Expected markdown string");
    assert.ok(
      this.lastResult.includes(text),
      `Markdown should contain "${text}". Got: ${this.lastResult.substring(0, 200)}`,
    );
  },
);

Then(
  "the markdown should not contain {string}",
  function (this: BmadWorld, text: string) {
    assert.ok(typeof this.lastResult === "string", "Expected markdown string");
    assert.ok(
      !this.lastResult.includes(text),
      `Markdown should NOT contain "${text}"`,
    );
  },
);
